...
impl WorldRepository {
    ...
    pub fn from_persisted_state(state: PersistedWorldRepository) -> Result<Self, String> {
        if state.schema_version != WORLD_REPOSITORY_SCHEMA_VERSION {
            return Err(format!(
                "unsupported datastore schema version: {} (expected: {})",
...
mod tests {
    use super::{
        ...
        CommitTarget, SphereEntity, TemporalWorldQuery, TimeWindow, WorldMutationFailure,
    ...
    #[test]
    fn from_persisted_state_rejects_outdated_schema_version() {
        let mut persisted = WorldRepository::new(example_world_snapshot()).to_persisted_state();
        persisted.schema_version = world_repository_schema_version().saturating_sub(1);

        let error = WorldRepository::from_persisted_state(persisted)
            .expect_err("outdated schema versions should be rejected");
        assert!(error.contains("unsupported datastore schema version"));
    }
